...
app.delete("/api/v1/details/:id", (req,res)=>{
   ...
   const restUser = userDetails.filter((el)=> el.id !==  parseInt(id));
    fs.writeFileSync(`${__dirname}/data/userDetails.json`,JSON.stringify(restUser));
    res.status(200).send({"status": "success","message": "User details deleted successfully", "data": {"details": user}})
})
